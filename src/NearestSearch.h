...
class IncrementalNeighbors {
 ...
 public:
  IncrementalNeighbors(const int query_point,
...
// incrementally search neighbors using kgraph, if not the target one ,jump to
// neighbor to search.
void IncrementalNeighbors::findMoreNeighbors(const int next) {
  vector<float> new_candidate_dists;
  vector<int> new_candidate_idxes;

  vector<float> query(p_nodes->at(query_point));

  vector<int> new_neighbors_list;
  for (auto cur_node : neighbors_list) {
    for (auto point : p_idxes->at(cur_node)) {
      if (candidates.find(point) == candidates.end()) {
        if (point == query_point) {
          continue;
        }
        candidates.insert(point);
        new_neighbors_list.emplace_back(point);
        new_candidate_idxes.emplace_back(point);
        float dist = EuclideanDistance(query, p_nodes->at(point));
        new_candidate_dists.emplace_back(dist);
      }
    }
  }
  hop += 1;
  ...
  new_candidate_dists.insert(new_candidate_dists.end(),
                             old_candidate_dists.begin(),
                             old_candidate_dists.end());
  new_candidate_idxes.insert(new_candidate_idxes.end(), sorted_idxes.begin(),
                             sorted_idxes.end());

  sort_candidates(new_candidate_dists, new_candidate_idxes);

  old_candidate_dists.assign(new_candidate_dists.begin(),
                             new_candidate_dists.end());
  sorted_idxes.assign(new_candidate_idxes.begin(), new_candidate_idxes.end());

  // sorted_idxes.insert(sorted_idxes.end(), sorted_new_candidate_idxes.begin(),
  //                     sorted_new_candidate_idxes.end());
  cur_neighbors = candidates.size();
  // cout << cur_neighbors << endl;
}

int IncrementalNeighbors::nextNeighbor(const int next) {
  if (cur_neighbors < next + 1) {
    findMoreNeighbors(next + 1);
  }
  return sorted_idxes[next];
}

bool IncrementalNeighbors::DetermineEarlyEnd(const int k_index) {
  int left_max = 0;
...
std::pair<int, int> IncrementalNeighbors::nndecentNeighbors(const int hop_limit,
                                                            const int k_index) {
...
vector<int> IncrementalNeighbors::findAllNeighbors(const int hop_limit,
                                                   const int K_index) {
  timeval t1, t2;
  ...
  sort_candidates(old_candidate_dists, sorted_idxes);
...
vector<int> IncrementalNeighbors::findHNSWNeighbors(const int K_index) {
  timeval t1, t2;
  gettimeofday(&t1, NULL);
  ...
  sort_candidates(dist_arr, second_sorted_idxes);
  sorted_idxes.assign(left_candidate_idxes.begin(), left_candidate_idxes.end());
  sorted_idxes.insert(sorted_idxes.end(), second_sorted_idxes.begin(),
                      second_sorted_idxes.end());
  sorted_idxes.insert(sorted_idxes.end(), right_candidate_idxes.begin(),
...
std::pair<int, int> IncrementalNeighbors::nndecentNeighbors(
...
vector<int> IncrementalNeighbors::findAllNeighbors(const vector<float> &query,
                                                   const int hop_limit,
                                                   const int K_index) {
  neighbors_list.clear();
  // add initial points
  int interval = (p_idxes->size()) / 3;
