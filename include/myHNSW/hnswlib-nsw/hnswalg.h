...
//                    if (candidate_id == 0) continue;
        ...
        void getNeighborsByHeuristic2(
                std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> &top_candidates,
        const size_t M, bool keepPrunedConnections = false) {
            ...
            std::priority_queue<std::pair<dist_t, tableint>> Wd;
            ...
            while (queue_closest.size()) {
                ...
                // no heuristic, the same to nsw
                if (good) {
                    return_list.push_back(curent_pair);
                }
                else {
                    Wd.emplace(-curent_pair.first,curent_pair.second);
                }
            }
            ...
            if (keepPrunedConnections){
                while(Wd.size() && top_candidates.size()<M){
                    std::pair<dist_t, tableint> curent_pair = Wd.top();
                    top_candidates.emplace(curent_pair.first, curent_pair.second);
                    Wd.pop();            
                }
            }
...
//                        continue;
        ...
        void linkNeighbors(const void *data_point, labeltype label, vector<int> neighbors){
            ...
            for (int level = std::min(curlevel, maxlevelcopy); level >= 0; level--) {
                ...
                std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> top_candidates;
                dist_t dist = fstdistfunc_(data_point, getDataByInternalId(currObj), dist_func_param_);
        ...
        void checkIntegrity(){
            ...
            std::priority_queue<std::pair<dist_t, labeltype >>
        searchCompactKnn(const void *query_data, size_t k) {
            std::priority_queue<std::pair<dist_t, labeltype >> result;
            if (cur_element_count == 0) return result;
            ...
            std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> top_candidates;
            top_candidates=searchBaseLayerST<false,true>(
                        currObj, query_data, ef_construction_);

            getNeighborsByHeuristic2(top_candidates,k,true);
    ...
    void buildKNNFirstGraph(const std::vector<std::vector<float>> &nodes,
